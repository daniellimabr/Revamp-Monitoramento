...
TIMESTAMP_FILE = os.path.join(BASE_DIR, "ultimo_timestamp_q5.txt")
IMG_DIR        = os.path.join(BASE_DIR, "imagens")

TITULOS = {
    'cancelados': 'M5 — Acordos Cancelados por Assessoria',
    'quebrados':  'M5 — Acordos Quebrados por Assessoria',
}
...
def upload_github(png_path, nome_arquivo):
    ...
    payload = {"message": f"M5 {nome_arquivo}", "content": conteudo_b64, "branch": GITHUB_BRANCH}
    if sha:
        payload["sha"] = sha
...
def enviar_gchat(image_url, titulo, subtitulo, thread_name=None):
    ...
    card = {
        "cards": [{"header": {"title": titulo, "subtitle": subtitulo},
...
if __name__ == "__main__":
    ...
    if not os.path.exists(TIMESTAMP_FILE):
        raise FileNotFoundError(f"Timestamp não encontrado: {TIMESTAMP_FILE}\nExecute primeiro: python gerar_grafico_q5.py")

    with open(TIMESTAMP_FILE) as f:
        meta = json.load(f)

    pngs = meta['pngs']  # {'cancelados': 'grafico_q5_cancelados_TS.png', 'quebrados': '...'}

    for chave, fname in pngs.items():
        png_path = os.path.join(IMG_DIR, fname)
        if not os.path.exists(png_path):
            print(f"  AVISO: PNG não encontrado, pulando: {fname}")
            continue
        titulo = TITULOS.get(chave, f"M5 — {chave}")
        raw_url = upload_github(png_path, fname)
        enviar_gchat(
            image_url   = raw_url,
            titulo      = titulo,
            subtitulo   = f"Últimos 45 dias · {hoje}",
            thread_name = thread_name,
        )

    print("\nM5 enviado com sucesso.")
