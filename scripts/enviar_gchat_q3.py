...
TIMESTAMP_FILE = os.path.join(BASE_DIR, "ultimo_timestamp_q3.txt")
...
def upload_github(png_path, nome_arquivo):
    ...
    payload = {"message": f"M3 {nome_arquivo}", "content": conteudo_b64, "branch": GITHUB_BRANCH}
    if sha:
        payload["sha"] = sha
...
if __name__ == "__main__":
    ...
    if not os.path.exists(TIMESTAMP_FILE):
        raise FileNotFoundError(f"Timestamp não encontrado: {TIMESTAMP_FILE}\nExecute primeiro: python gerar_grafico_q3.py")

    with open(TIMESTAMP_FILE) as f:
        meta = json.load(f)

    pngs = meta['pngs']  # {'m3': 'grafico_q3_TIMESTAMP.png'}
    ...
    for chave, fname in pngs.items():
        ...
        enviar_gchat(
            image_url   = raw_url,
            titulo      = "M3 — Pagamentos por Meio",
            subtitulo   = f"Últimos 45 dias · {hoje}",
            thread_name = thread_name,
        )

    print("\nM3 enviado com sucesso.")
