...
TIMESTAMP_FILE = os.path.join(BASE_DIR, "ultimo_timestamp.txt")
...
def upload_github(png_path, nome_arquivo):
    ...
    payload = {"message": f"M1 {nome_arquivo}", "content": conteudo_b64, "branch": GITHUB_BRANCH}
    if sha:
        payload["sha"] = sha
...
if __name__ == "__main__":
    ...
    print(f"Enviando {len(pngs)} PNGs do M1...")
    ...
    for grp_key, fname in pngs.items():
        ...
        enviar_gchat(
            image_url = raw_url,
            titulo    = f"M1 — Contratos por Segmento  |  {subtitulo_grp}",
            subtitulo = f"Foto atual · {hoje}",
            thread_name = thread_name,
        )

    print("\nM1 enviado com sucesso.")
