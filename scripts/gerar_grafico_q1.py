...
arquivos = glob.glob(os.path.join(BASE_DIR, 'Query 1*.csv')) + \
           glob.glob(os.path.join(BASE_DIR, 'Query_1*.csv'))
if not arquivos:
    raise FileNotFoundError('Nenhum arquivo CSV com "Query 1" encontrado na pasta.')
...
TS_FILE         = os.path.join(BASE_DIR, 'ultimo_timestamp.txt')

# ── Expurgo dos PNGs anteriores ───────────────────────────────────────────────
expurgar("grafico_q1_")
garantir_pastas()
...
try:
    for grp,segs in GRUPOS_SEG.items():
        ...
        fname=f'grafico_q1_{grp}_{TIMESTAMP}.png'
        png_path=os.path.join(IMG_DIR,fname)
        titulo=(f'M1 · Contratos por Segmento — Foto Atual  |  '
    ...
    registrar(
        monitoramento="M1",timestamp=TIMESTAMP,status="ok",
    ...
    registrar(
        monitoramento="M1",timestamp=TIMESTAMP,status="erro",
        qtd_contratos=_qtd_log,vl_total=_vl_log,pngs=pngs_gerados,
